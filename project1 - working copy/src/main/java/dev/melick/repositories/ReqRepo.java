...
public class ReqRepo {
    ...
    public void updateAmt(Request r){

        try(Connection connection = getConnectionUtil().getConnection()){
            System.out.println("request received in ReqRepo updateAmt() method: \n" + r);
            String sql = "UPDATE requests SET final_amt = ?, benco_upgrade = ?, benco_downgrade = ?, why_benco_upgrade = ?, benco_app_by = ? WHERE req_id = ?";

            PreparedStatement ps = connection.prepareStatement(sql);

            ps.setBigDecimal(1, r.getFinalAmt());
            ps.setBoolean(2, r.isBencoUpgrade());
            ps.setBoolean(3, r.isBencoDowngrade());
            ps.setString(4, r.getWhyBencoUpgrade());
            ps.setInt(5,r.getBencoAppBy());
            ps.setInt(6, r.getReqId());
            System.out.println(ps);
            ps.executeUpdate();
